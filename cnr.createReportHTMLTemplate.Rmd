---
title: "<span style='font-size: 0px;'>Cutlery Report</span>"
date: "`r Sys.Date()`"
output:
    html_document:
    df_print: kable
...
```{r, echo=FALSE, out.width="35%", message=FALSE, warning=FALSE, fig.align='center'}
...
\
```{r PCA_frag_dist, echo=FALSE, out.width="100%", message=FALSE, warning=FALSE}
	#initialize df and sample name list for header labeling
	...
	p <- plot_ly(samplesDF, x=pcaSamples$x[,1], y=pcaSamples$x[,2], text=rownames(samplesDF),
				mode="markers", color = rownames(samplesDF), marker=list(size=11))
	p <- layout(p,title="Fragment Distribution PCA Analysis",
				xaxis=list(title="PC1 (% of Variance Explained)"),
				yaxis=list(title="PC2 (% of Variance Explained)"))
	p
