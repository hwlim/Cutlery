...
def get_peak_mode(sampleName):
	modeName = samples.PeakMode[samples.Name == sampleName]
	return modeName.tolist()[0]

rule draw_peak_examples_histone:
	input:
		peakFile = sampleDir + "/{sampleName}/HomerPeak.histone/peak.exBL.bed"
	output:
	...
	shell:
		...
		cnr.visualizePeakExamples.r -o {sampleDir}/{wildcards.sampleName}/HomerPeak.histone \
			-m {params.peakMode} -c {params.ctrlSampleName} -n {params.numPeaks} {input.peakFile}
		"""

rule draw_peak_examples_factor:
	input:
		peakFile = sampleDir + "/{sampleName}/HomerPeak.factor/peak.exBL.1rpm.bed"
	output:
